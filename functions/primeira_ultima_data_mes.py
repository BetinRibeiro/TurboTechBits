...
def calcular_mes_referencia(meses):
    ...
    # Calcula o primeiro dia do mês de referência corretamente
    primeiro_dia = (data_atual + relativedelta(months=meses)).replace(day=1)
    
    # Obtém o último dia do mês de referência
    ultimo_dia = primeiro_dia.replace(day=calendar.monthrange(primeiro_dia.year, primeiro_dia.month)[1])
    
    return primeiro_dia.date(), ultimo_dia.date()
